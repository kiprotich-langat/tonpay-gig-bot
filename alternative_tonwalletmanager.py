...
#             # Load compiled contract code from escrow.boc
#             contract_code = await self._load_contract_code()
...
#     async def _load_contract_code(self) -> Cell:
#         """Load compiled escrow contract code from escrow.boc"""
#         try:
#             # Look for escrow.boc in multiple locations
#             possible_paths = [
#                 Path("escrow.boc"),
#                 Path("contracts/escrow.boc"),
#                 Path("../escrow.boc"),
#                 Path("./contracts/escrow.boc")
#             ]
...
#                         boc_data = f.read()
#                         cells = Cell.from_boc(boc_data)
#                         logger.info(f"✅ Contract code loaded successfully ({len(boc_data)} bytes)")
#                         return cells[0]
            
#             # If not found, raise error
#             raise FileNotFoundError(
#                 f"escrow.boc not found in any of these locations:\n" + 
#                 "\n".join(f"  - {p}" for p in possible_paths) +
#                 "\n\nPlease place your compiled escrow.boc file in the project root or contracts/ folder"
#             )
            
#         except Exception as e:
#             logger.error(f"Failed to load contract code: {e}")
#             raise
...
#                     diagnostics['contract_code_path'] = str(path)
#                     break
            
#             if not diagnostics['contract_code_found']:
#                 diagnostics['issues'].append("escrow.boc not found")
#         except Exception as e:
#             diagnostics['issues'].append(f"Error checking contract: {e}")
...
#             diagnostics['contract_code_found'] and
...
#         print(f"   Contract Code: {'✅' if diagnostics['contract_code_found'] else '❌'}")
